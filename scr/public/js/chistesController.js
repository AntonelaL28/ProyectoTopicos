    ...
    content.innerHTML = `
      <section>
        ...
        <main class="contendorFormulario">
          <label class="texto">
            Categoría:
            <input type="text" id="categoriaCantidad" required>
          </label>
...
async function obtenerCantidadChistesPorCategoria() {
  ...
  try {
    ...
    mostrarResultadoCantidadChistes(data);
  } catch (error) {
    alert("Ocurrió un error al obtener la cantidad de chistes por categoría");
  }
}

function mostrarResultadoCantidadChistes(data) {
  const contenedorResultado = document.getElementById('resultadoCantidadChistes');
  contenedorResultado.innerHTML = `Hay ${data.cantidad} chistes en la categoría ${data.categoria}`;
}
