...
async function actualizarChiste() {
  ...
  let chisteActualizado = await obtenerID();
  botonActualizar.disabled = false;

  botonActualizar.addEventListener("click", async () => {
    efectoEncima(botonActualizar, "#575454");
    try {
      chisteActualizado.TxtChiste = btTxt.value;
      chisteActualizado.NomUser = btUser.value;
      chisteActualizado.Puntaje = btPuntaje.value;
      chisteActualizado.Categoria = btCategoria.value;
      ...
      const response = await fetch(`http://localhost:3005/actualizarChiste/${btID.value}`, {
        ...
        body: JSON.stringify(chisteActualizado) 
      });
